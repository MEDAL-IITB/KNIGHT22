...
def prepare_clinical(sample_dict: dict) -> dict:
    ...
    # print(sample_dict['data'].keys())
    # print(sample_dict['data']['input'].keys())
    # print(sample_dict['data']['input']['clinical'].keys())
    # exit()
    try:
        try:
        ...
        print("skipping")
    features = FuseUtilsHierarchicalDict.get(sample_dict, 'data.input.clinical.features')
    # age = FuseUtilsHierarchicalDict.get(sample_dict, 'data.input.clinical.age_at_nephrectomy')
    ...
    # exit()
    clinical_encoding = torch.cat((age, bmi, radiographic_size, preop_egfr, gender_one_hot, comorbidities_one_hot, smoking_history_one_hot, features), dim=0)
...
def knight_dataset(data_dir: str = 'data', cache_dir: str = 'cache', split: dict = None, \
    ...
    if 'train' in split:
        ...
        json_filepath = os.path.join(data_dir, 'knight.json')
        gt_processors = {
